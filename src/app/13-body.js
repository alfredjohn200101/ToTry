// ── WEIGHT UNITS ─────────────────────────────────────────────────────────────────────────────
// Storage is ALWAYS kilograms. The whole app's maths depends on it — Mifflin-St Jeor takes kg, the
// recomp read compares kg, the CSV importers convert lb to kg on the way in. Nothing below changes a
// stored number; this layer converts only at the two edges, display and entry.
//
// Before this existed, the weight-unit setting relabelled exactly ONE line and converted nothing, so
// picking "lb" showed a person their 82.4kg as "82.4 lb" — a real number, a real unit, and the wrong
// reading by a factor of 2.2 — while "82.4kg" sat three lines below it on the same screen. Worse, a
// person typing 180 meaning pounds had 180 stored and fed to the calorie maths as kilograms, which
// produces a confidently wrong target. The setting was honest about this in its own label ("lb (label
// only)") rather than sweeping half the sites, which was the right call until someone did the work.
//
// Body weight only. A lifted load is a different quantity and stays in kg — see the note on the
// setting itself.
...
const _LB_PER_KG = 2.20462262;
function wUnit(){ return (ls('totry_weight_unit') === 'lb') ? 'lb' : 'kg'; }
// kg → whatever the person reads in
function kgToDisp(kg){
  const n = parseFloat(kg);
  if(!isFinite(n)) return n;
  return wUnit() === 'lb' ? Math.round(n * _LB_PER_KG * 10) / 10 : n;
}
// what the person typed → kg. Rounded to 2dp so a lb entry round-trips to the same tenth it was
// typed as, instead of drifting by a gram every time it is redisplayed.
function dispToKg(v){
  const n = parseFloat(v);
  if(!isFinite(n)) return n;
  return wUnit() === 'lb' ? Math.round((n / _LB_PER_KG) * 100) / 100 : n;
}
// The one formatter. Pass kg, get the string a person should see.
function wFmt(kg, opts){
  opts = opts || {};
  const n = parseFloat(kg);
  if(!isFinite(n)) return opts.blank != null ? opts.blank : '\u2014';
  const d = kgToDisp(n);
  const r = (opts.round === 0) ? Math.round(d) : Math.round(d * 10) / 10;
  const sign = (opts.signed && r > 0) ? '+' : '';
  return sign + r + (opts.bare ? '' : wUnit());
}
// A CHANGE in weight converts as a magnitude — never call kgToDisp on a difference and then add a
// unit by hand, because a delta of 0 must stay 0 and a negative must stay negative.
function wDelta(kgDiff, opts){
  opts = opts || {};
  const n = parseFloat(kgDiff);
  if(!isFinite(n)) return opts.blank != null ? opts.blank : '\u2014';
  if(n === 0) return opts.zero != null ? opts.zero : ('0' + wUnit());
...
function adjustCaloriesFromWeightTrend(){
  ...
  if(weeklyChange < targetMin){
    ...
    if(userGoal === 'lose'){
      ...
      reason = 'You\'re dropping ' + wFmt(Math.abs(weeklyChange)) + '/week (target: ' + wFmt(0.3,{bare:true}) + '-' + wFmt(0.9) + '). Bumping your calories up.';
    } else if(userGoal === 'maintain'){
      calAdjustment = 200;
      reason = 'You\'ve lost ' + wFmt(Math.abs(weeklyChange)) + '/week. Bumping calories up to maintain.';
    } else {
...
function renderWeightHistory(){
  ...
  // EVERY WEIGH-IN HAS TO BE REACHABLE. This showed the 8 newest and nothing else, and the delete
  // control lives on the row — so the 9th weigh-in onward could never be corrected or removed,
  // however wrong it was. deleteWeightEntry() handles any key; only the render withheld it.
  const _shown = _weighShowAll ? entries : entries.slice(0,8);
  const _thisYear = new Date().getFullYear();
  wrap.innerHTML = '<div class="lbl" style="margin-bottom:6px">Recent weigh-ins</div>' +
    _shown.map(e => {
      // ...and dated unambiguously. "5 Sep" on two entries a year apart is the same label twice.
      const _d = e.ts ? new Date(e.ts) : null;
      const _opts = (_d && _d.getFullYear() !== _thisYear)
        ? {day:'numeric',month:'short',year:'numeric'} : {day:'numeric',month:'short'};
      const when = _d ? _d.toLocaleDateString('en-AU',_opts) : (e.date||'');
    ...
    // The way back to the rest of them. Without this the list simply stopped at eight with no
    // sign there were more, which is why nobody noticed the older ones were unreachable.
    (entries.length > 8
      ? '<button onclick="toggleWeighShowAll()" style="width:100%;min-height:34px;margin-top:6px;background:none;border:none;color:var(--tx3);font-size:11.5px;font-family:\'DM Mono\',monospace;cursor:pointer">'
        + (_weighShowAll ? 'Show fewer' : 'Show all ' + entries.length + ' weigh-ins') + '</button>'
      : '');
}
...
function renderBodyGoalCard(){
  ...
  if(p.kind === 'there'){
    ...
    // They set a target, reached it, and kept going. That is not drifting away from a goal.
    head = 'You went past it.'; col = 'var(--gr)';
    body = 'You set ' + wf(p.goal) + ' and you are at ' + wf(p.now) + ' \u2014 ' +
           ((typeof wFmt === 'function') ? wFmt(p.by) : (Math.round(p.by*10)/10 + 'kg')) +
           ' beyond it. If this is where you meant to end up, set the new number so the app is ' +
           'aiming where you are.';
...
function bodyGoalProjection(){
  try{
    ...
    // HAVING PASSED THE GOAL IS NOT THE SAME AS DRIFTING FROM IT. The `moving` test compares the sign
    // of the gap with the sign of the trend and nothing asks whether the goal was already CROSSED —
    // so someone who set 80kg, reached it, and kept going to 78 has a gap that flipped sign, and the
    // card flips with it to "going away from your goal". They are not going away from it; they went
    // past it, which is the thing they were trying to do. Say that instead.
    const _startW = (function(){ try{ const w=(ls('totry_body')||[]).filter(e=>e&&(parseFloat(e.weight)||0)>0);
      return w.length ? parseFloat(w[w.length-1].weight) : null; }catch(_){ return null; } })();
    const _passed = (_startW != null) && ((_startW < goal && now > goal) || (_startW > goal && now < goal));
    if(_passed) return { kind:'passed', now:now, goal:goal, by:Math.abs(gap) };
...
function renderBody(){
  ...
  const todayFull = new Date().toLocaleDateString('en-AU');                                   // identity
    ...
    if(todayEntry && todayEntry.weight > 0){
      ...
      todaySummary.textContent = 'Last: ' + wFmt(entries[0].weight) + ' (' + (entries[0].date || (entries[0].ts ? new Date(entries[0].ts).toLocaleDateString('en-AU') : 'earlier')) + ')';
  ...
  if(weighed.length){
    ...
    if(bmiEl){
      if(h > 0){
        ...
        // A working button 13px tall. It has a role and a handler; it needed a box a thumb can find.
        bmiEl.style.padding = '7px 10px';
        bmiEl.style.minHeight = '24px';
        bmiEl.style.display = 'inline-flex';
        bmiEl.style.alignItems = 'center';
        bmiEl.style.justifyContent = 'center';
        bmiEl.setAttribute('tabindex','0');
        bmiEl.onkeydown = function(e){ if(e.key==='Enter'||e.key===' '){ e.preventDefault(); bmiEl.onclick(); } };
  ...
  if(hist){
    ...
    entries.forEach((e,i)=>{
      ...
      let html='<div style="display:flex;justify-content:space-between;align-items:baseline;margin-bottom:'+((scoreLine||e.win||e.struggle||e.focus||e.note)?'8px':'0')+'">'+
        ...
        (e.weight>0?'<span style="font-size:15px;color:var(--tx)">'+wFmt(e.weight)+' <span style="font-size:11px;color:'+(cls==='down'?'var(--gr)':cls==='up'?'var(--re)':'var(--tx3)')+'">'+cs+'</span></span>':'')+
        '</div>';
...
function saveQuickWeight(){
  const w = dispToKg(parseFloat(document.getElementById('quick-weight-in')?.value || 0));   // stored in kg, always
  if(!w || w < 20 || w > 400){
    showToast('Enter a real weight', 'Between ' + wFmt(20, {round:0}) + ' and ' + wFmt(400, {round:0}) + '.');
    return;
  }
  const entries = ls('totry_body') || [];
  const todayStr = new Date().toLocaleDateString('en-AU', {day:'numeric', month:'short'});  // display only
...
function updateTrackerDisplay(){
  ...
  if(sum){
    if(todayWeight){
      // This used to append the unit LABEL to the stored kilograms, so a person on lb was told
      // their 82.4kg was "82.4 lb" — the wrong reading by 2.2x, contradicted by the kg figure three
      // lines below it. wFmt converts.
      sum.textContent='Logged today: '+wFmt(todayWeight.weight);
